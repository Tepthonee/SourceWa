...
Module({pattern: 'تاك ?(.*)',use: 'group', fromMe: true, desc: Lang.TAGALL_DESC}, (async (message, match) => {
...
Module({pattern: 'توجيه ?(.*)',use: 'utility', fromMe: true, desc: "Forwards message"}, (async (message, match) => {
...
Module({pattern: 'ارسال ?(.*)',use: 'utility', fromMe: true, desc: "Sends message"}, (async (message, match) => {
    if (!message.reply_message) return await message.sendReply("*Reply to a message*\n*Ex: .send jid jid ...*")
